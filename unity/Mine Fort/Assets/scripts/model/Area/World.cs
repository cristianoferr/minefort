...
public class World
{
    ...
    {
        return mapData.GetTileAt(x, y,z);
    }
    public Tile GetTileAt(Vector3 pos)
    {
        return mapData.GetTileAt(pos.x, pos.y,pos.z);
    }
    ...
    public void TilesFromJson(JToken tilesToken)
    {
        ...
        foreach (JToken tileToken in tilesJArray)
        {
            ...
            GetTileAt(x, y, z).FromJson(tileToken);
        }
    }
    ...
    {
        Utils.LogError("ERROR: ReadJson not implemented");
    ...
    {
        Utils.LogError("ResizeWorld not implemented");
        ...
        Tile[,,] oldTiles = (Tile[,,])mapData.tiles.Clone();
    ...
    private void FillTilesArray()
    {
        for (int x = 0; x < Width; x++)
        {
            for (int y = 0; y < Height; y++)
            {
                ...
                {
                    Tile tile = GetTileAt(x, y, z);
                    tile.TileChanged += OnTileChangedCallback;
                    tile.TileTypeChanged += OnTileTypeChangedCallback;
                    tile.Room = RoomManager.OutsideRoom; // Rooms 0 is always going to be outside, and that is our default room
                }
            }
        }
    }
