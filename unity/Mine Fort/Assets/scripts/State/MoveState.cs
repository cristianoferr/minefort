...
namespace MineFort.Entities.States
{
    public class MoveState : State
    {
        ...
        public MoveState(GameCharacter character, Pathfinder.GoalEvaluator goalEvaluator, List<Tile> path, State nextState = null)
        ...
        {
            if (nextTile.IsEnterable() == GameConsts.ENTERABILITY.Soon)
            {
            ...
            if (movementPercentage >= 1f)
            {
                ...
                else if (nextTile.IsEnterable() == GameConsts.ENTERABILITY.Never)
                {
